...
player_info_df = original_df.select("id", "player", "year", "stint", "team", "lg").distinct()
...
player_info_df.write.format("delta").mode("overwrite").save(player_info_table_path)
...
# Cast the 'id' column to the desired data type (e.g., IntegerType) in game_stats_df
game_stats_df = original_df.select(
    "id", "year", "team",  "r", "h", "X2b", "X3b", "hr", "rbi", "sb", "cs", "bb", "so", "ibb", "hbp", "sh", "sf", "gidp"
).distinct()
